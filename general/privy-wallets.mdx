...
This wallet is the root of ownership for everything in HEVN: it is the sole owner of the user's [Base Smart Wallet](/general/smart-wallets), where the actual balance lives.
...
The result: **neither Privy nor HEVN ever sees the user's key**. HEVN cannot sign on the user's behalf, and a compromise of HEVN's servers would expose no key material at all.
...
There is no password stored by HEVN that could be leaked, and no seed phrase that could be phished. Conversely, this makes the email account the crown jewel — see [Security model](/general/security-model#protect-your-email) for recommended protections (2FA, hardware keys).
