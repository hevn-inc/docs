...
Every funding rail in HEVN resolves to the same place: the user's own [Base Smart Wallet](/general/smart-wallets). HEVN requests the details from a partner bank, orchestrates metadata, and shows status — but it is never a party in the money flow.

## Deposits

### Virtual account details are bound to your wallet

When a partner bank opens virtual account details (account numbers / IBANs) for a user, those details are linked **only to that user's smart wallet address**. An incoming fiat transfer is converted by the banking partner and settles as USDC directly on the user's wallet.
...
<Steps>
  ...
  <Step title="The banking partner issues a one-time crypto address">
    The licensed banking partner returns a **single-use deposit address** dedicated to this specific payout. The address travels through HEVN's isolated [provider microservice](/general/architecture#provider-isolation-signed-responses), which signs it — the client verifies that signature before ever showing you the address.
  </Step>
...
- **Tamper-evidence** — the address is signed by the isolated provider service and verified by the client, so it cannot be substituted anywhere along the way ([details](/general/architecture#provider-isolation-signed-responses)).
