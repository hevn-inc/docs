...
| Grants | Contacts, invoices, contracts, virtual account details, KYC, statements, analytics | Transfers and any onchain state change |
...
The JWT authorizes the HEVN API surface: creating contacts, issuing and paying *requests* for invoices, generating contracts, requesting virtual account details, reading balances and statements.
...
For automation, HEVN issues app-scoped `hvn_...` API keys (see [CLI authentication](/cli/overview#authentication-model)). They are a CLI and agent credential: the [whitelabel REST API](/api-reference/authentication) uses developer keys and JWT sessions instead, and rejects `X-Api-Key`. An API key is more than a bearer token — it is a **full EVM-compatible keypair**, and its spending ability is governed onchain, by the same standard as [teammate access](/general/shared-access):

1. **Platform scope.** The key is certified by HEVN for the platform API of one specific app: contacts, invoices, contracts, virtual account details, statements. This is the same trust level as the JWT.
