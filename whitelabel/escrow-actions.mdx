...
HEVN re-checks the developer key registration behind the session immediately before it co-signs: a key paused between prepare and confirm, or a confirm sent from an IP outside the key's allowlist, answers `403` and signs nothing. The prepared action is untouched, so a confirm from an allowlisted host still works.
...
| `403 forbidden` | The developer key registration was paused, or the confirm left an IP outside its allowlist. | Nothing was signed. Register a key that covers this host, or resume from an allowlisted one. |
