...
Six actions, one resource. `POST /v1/escrow/{escrowId}/actions` prepares one and returns a payload to sign; `POST /v1/escrow/{escrowId}/actions/{idempotencyKey}/confirm` submits the signature. This page is the matrix.
...
`approve` is not a verb you send: it is prepared for you by `POST /v1/escrow` and confirmed under the create's own key. Sending an `amount` with `void` or `reclaim` answers `409 amount_not_allowed`; omitting it on the four that need it answers `422 validation_failed`.
...
```json Request — POST /v1/escrow/esc_5Qb4e17c9d/actions, Idempotency-Key: order-A-1187-capture
...
```json Request — POST /v1/escrow/esc_5Qb4e17c9d/actions/order-A-1187-capture/confirm
...
| `202`, `pollUrl` | With the bundler, no receipt yet. | `pollUrl` is the deal read, `GET /v1/escrow/{escrowId}`; `POST /v1/escrow/{escrowId}/sync` is the one that reconciles against the chain. Sync until the action settles. |
...
| `authorized` | The hold is in the contract. `capturableAmount` is positive. |
...
`GET /v1/escrow/{escrowId}` returns one deal with its last action. `GET /v1/escrow` lists the deals you operate, newest first, cursor-paged:
...
`POST /v1/escrow/{escrowId}/sync` reconciles the deal against the chain and returns the same shape plus `consistent`. It reads the chain at most once every 15 seconds per deal; calls in between answer the stored projection, which is why polling it in a tight loop buys nothing.
...
<Card title="Next: Sandbox" icon="flask" href="/whitelabel/sandbox">
  The same 48 operations against an emulated partner on Base Sepolia.
</Card>
