...
- Your developer key still exists, holds the `escrow:sign` scope, and the call leaves an allowlisted source IP. All three are re-checked on every request, and again immediately before HEVN co-signs an action — see [Developer key](/whitelabel/developer-key#scopes).
