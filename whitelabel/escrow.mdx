---
title: "Escrow"
description: "Hold one client's money while another delivers: the states a deal passes through, the windows it lives in, and a marketplace order end to end."
---

import IdempotencySnippet from "/snippets/idempotency.mdx";

Escrow lets you hold one client's money while another delivers. The buyer's money stays in the buyer's own wallet until you authorize a hold, it sits in the escrow contract where neither party can spend it, and it reaches the seller only when you capture.

```mermaid
stateDiagram-v2
    [*] --> draft: POST /v1/escrow
...
Every transition is one `POST /v1/escrow/{escrowId}/actions` that you sign with your developer key. You are the **operator** of every deal you create; escrow is the one resource that never takes `X-Hevn-Account`, because the caller is always you.
...
All three must be different accounts, and both parties must be clients you created — a stranger's account id answers `422 receiver_not_a_client` or `404 client_not_found`. The addresses are each party's Base smart wallet at creation time, and the deal is immutable: its terms are hashed into an on-chain payment record.
...
<Steps>
  <Step title="Open the deal and approve the spend permission">
    ...
    ```json Response — 201, Location: /v1/escrow/esc_5Qb4e17c9d
    {
      "id": "esc_5Qb4e17c9d",
      "status": "draft",
  ...
  <Step title="Hold the buyer's money">
