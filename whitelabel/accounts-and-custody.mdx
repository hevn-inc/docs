...
flowchart TB
    ...
    CA -.->|"legal identity and requisites"| W
```
...
`GET /v1/clients/{clientId}/balance` reads, and what every payout debits. Client wallets are separate
...
never exists outside [Privy's key management](/general/privy-wallets). Two things must happen before
that EOA signs anything:

1. **You sign.** Your developer key is registered as a signer for the wallet, and signs the exact
   operation HEVN prepared.
2. **HEVN co-signs.** The signer is a two-of-two quorum: your key and HEVN's own authorization key.
...
| Can the client move its own balance? | Not through this API: nothing in it lets a client spend, and a client never signs a payment you make. You are its treasury. |
...
| Does HEVN decide who a client may pay? | The partner bank does, through the refusals a payout can return. HEVN never picks a recipient. |
...
You are the custody surface your customer sees. Two obligations follow.
...
you register a new key in the app and keep signing. A *leaked* key is different. Until you rotate it,
whoever holds it can ask HEVN to build a payment from any of your clients' wallets, and HEVN will
co-sign it, because that is exactly what the key means.

<Warning>
  Treat the developer key as a spending credential for every client balance at once. Keep the private
  half in a secret manager, never in a browser, never in a repository, and rehearse
...
with `X-Hevn-Account: cl_…` added — one token, any client. What that header can and cannot reach is
