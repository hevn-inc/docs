...
Money arrives four ways: a quoted deposit you priced, a quoted on-chain transfer you priced, an unsolicited wire to open requisites, or crypto sent straight to the wallet. All four end as one income row on the client's ledger.
...
| **Unsolicited wire** | Nothing — the requisites stay live | The `requisites` on the client's active rail from `GET /dapi/v1/banks` |
...
The client needs an **active rail** in the currency the payer sends — see [Virtual accounts](/whitelabel/bank-accounts).
...
<Steps>
  <Step title="Price the deposit">
    ...
    ```json Response — 201, Location: /dapi/v1/payins/pi_2b7…
    ...
    - **`settlementToken`** is the stablecoin the payin is delivered in, `USDC` (the default) or `EURC`. It has to be a token one of the client's matching rails settles into; when it is not, the call refuses with the pairs that work (see below).
...
```json
{ "amount": "2000.00", "currency": "USDC", "originChainId": "arb", "settlementToken": "USDC" }
```
...
An active rail's requisites stay live between payments. Anyone can wire to them with no quote and no API call from you.
...
3. The deposit lands as an income row on `GET /dapi/v1/transactions`, carrying `remitter` where the partner reports the sender, `paymentReference`, and the receiving requisites.
