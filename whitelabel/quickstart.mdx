...
<Steps>
  ...
  <Step title="Prepare the payout">
    ...
    no conversion, and `quote` carries no rate and no fee. `debit` is what will actually leave the
    wallet, decoded from the operation you are about to sign, and for a wallet recipient its
    `address` is the recipient. `approval.payload` is the bytes to sign, live for 120 seconds.
