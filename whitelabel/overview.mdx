---
title: "Overview"
description: "Issue real bank accounts to your own customers and move their money from your backend, with one API and one key."
---

With the whitelabel API you issue real bank accounts to your own customers and move their money from
your backend — no HEVN UI, no HEVN operator, no end-user signature.
...
flowchart TB
    ...
    subgraph Wallet["Client account — one per customer"]
        ...
        T["Contacts<br/>saved payout recipients"]
...
- **A marketplace with escrow.** Hold a buyer's money while a seller delivers, then capture it minus
  your fee.
...
- **An embedded neobank.** Give each of your customers bank details in their own company name, a
  balance, a statement and outgoing payments, under your brand.
...
identity you verify once, bank **rails** that produce requisites in its own name, a balance, and a
ledger.
...
your server. The same key authorizes money movement: HEVN builds each payment, echoes its exact
terms, you check them and sign, HEVN executes. Nothing is ever constructed by you, and nothing moves
without your signature. See [Signing](/whitelabel/signing).

A client becomes usable by passing four gates in order — created, verification document complete,
application approved, rail active. After that, one payment is two calls: `POST /v1/payouts` books and
prices it, `POST /v1/payouts/{payoutId}/confirm` signs it.

## What you get from HEVN, and what you keep

| You build | HEVN provides |
| --- | --- |
| Your product, your UI, your customer relationship | The accounts, the bank rails, the balances, the ledger |
| The onboarding form your customer fills in | The verification review and the partner-bank submission behind it |
...
<CardGroup cols={3}>
  ...
  <Card title="Accounts and custody" icon="shield" href="/whitelabel/accounts-and-custody">
