...
| --- | --- |
| `POST /v1/payouts` | the booking and its price. A retry replays; different terms answer `409 idempotency_key_reused` with `details.payoutId`. |
| `POST /v1/transfers` | the transfer. The key is part of the `tr_` id, so the same key always names the same transfer. |
