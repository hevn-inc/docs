...
login limiter — it is an ordinary write, and it re-checks the developer key registration behind the session before it
mints anything.
...
| `POST /dapi/v1/contacts`, `PATCH`, `DELETE /dapi/v1/contacts/{contactId}` | nothing: a contact is deduplicated by its own requisites. |
...
| `publicKey` on a developer key registration | 4,096 characters |
| `ipAllowlist` on a developer key registration | 1 to 64 addresses or CIDR networks |

There is no `offset` anywhere. Walk a list by passing `nextCursor` back as `cursor` until it is absent.

The two developer-key caps are the only ones you cannot raise by calling again: `ipAllowlist` and `scope` are frozen
when the registration is created, so a network the allowlist does not cover needs a second registration, not an edit.
The `publicKey` cap there is the DER-encoded public half, which is a different field from the 512-character
`publicKey` a payout carries.
