...
| `recipient_chain_unsupported` | 422 | The recipient is on a chain a client balance cannot pay. Only USDC on Base is paid from the balance; `details.chain` echoes what was asked. |
| `recipient_token_unsupported` | 422 | The recipient expects a token a client balance cannot pay. `details.token` echoes it. |
...
These arrive at `POST /v1/payouts/{payoutId}/confirm`; two of them — `quote_not_submitted` and `payout_not_fundable` — also answer the sandbox settlement routes, where they mean the same thing. The decision table for the four that branch is on [Payouts](/whitelabel/payouts#when-a-payout-is-refused).
