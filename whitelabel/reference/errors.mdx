---
title: "Errors"
description: "One envelope and all 109 error slugs, grouped by resource, with what to do about each."
---
...
| `forbidden` | 403 | This session may not perform this operation. On `/dapi/v1` it is also what a non-platform token, a paused developer key registration and an off-allowlist source IP answer — see [Developer keys](#developer-keys). |
...
| `key_not_registered` | 403 | This public key is not in the account's signer, or has no registration of its own. See [Developer key](/whitelabel/developer-key). |
...
A platform session is established by one registered developer key, and every `/dapi/v1` request re-reads that
registration: it must still be active, and the request must arrive from an address inside the key's allowlist. Both
checks are repeated on `POST /dapi/v1/auth/refresh` and again immediately before HEVN co-signs, so pausing a key stops
the sessions it already minted rather than waiting for them to expire.
...
| `forbidden` | 403 | With no `details`: the registration behind the session is paused, or the call left an address outside its allowlist. Resume from an allowlisted host, or register a key that covers this one. |
| `key_not_registered` | 403 | The public key you named in a signature has no registration on this account. |
| `invalid_credentials` | 401 | At login only. A paused key, an off-allowlist IP, an unknown email and a bad signature are all one code, deliberately. |
| `developer_key_not_found` | 404 | `POST /api/v1/developer/api-keys/{developerKeyId}/pause` was given a `dk_…` id that is not yours. |
| `developer_key_already_exists` | 409 | That public key is already registered for this developer. Registrations are immutable and additive — pause the old one instead of re-registering. |

The last two come from the management routes under `/api/v1/developer/api-keys`, which take an ordinary owner JWT and
carry the ordinary API's own error shape. They are deliberately absent from `/dapi/v1`.
...
| Slug | Status | What to do |
| --- | --- | --- |
| `contact_details_invalid` | 422 | The contact's requisites or address are incomplete for the method, at create or update time. `details.fields` names them. |
...
| `self_transfer` | 409 | The contact is the client's own wallet. |
