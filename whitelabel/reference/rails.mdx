...
Per deposit the choice is on the money-in call: `POST /v1/payins` takes a `settlementToken`, and a token no rail of the client's settles into answers `422 payin_not_available` with `details.options[]` listing the pairs that do.
