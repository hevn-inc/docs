...
| --- | --- |
| The developer keypair and its registration | Keys are registered per environment. Generate a fresh P-256 key for production, add its public half to that account's signer, then register it with `POST /api/v1/developer/api-keys` carrying the **production** egress IPs in `ipAllowlist`. The sandbox key, and the sandbox allowlist, are both unknown in production. |
...
The allowlist is the second variable that fails this way. A registration is pinned to the source IPs you gave it and cannot be edited, so a production deploy behind a different NAT answers `403` on every call although the key, the signature and the token are all correct. Enumerate the egress addresses of every host that will call HEVN — including the ones an autoscaler adds — before you register the production key.
...
| Rail activation | `active` with requisites immediately | `preparing` until the partner publishes the account details. Poll `GET /dapi/v1/banks` |
...
<Steps>
  ...
  <Step title="Register a production developer key">
    A new P-256 keypair, generated on the machine that will hold it, mode `0600`. Add the public half to the production account's signer, then register it with `POST /api/v1/developer/api-keys` and the production egress IPs. Never copy the sandbox key, and never assume the sandbox allowlist carries over.
  </Step>
  ...
  <Step title="Rehearse losing the key">
    Register a second key before you need it, book a payout naming the new `publicKey`, and confirm it. Rotation is a field on `POST /dapi/v1/payouts`, not a migration — but only if the second key is already registered **and** its allowlist covers the hosts that will use it. Pause the old registration once the new one has carried a payment.
  </Step>
