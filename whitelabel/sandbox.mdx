---
title: "Sandbox"
description: "The one place /v1/sandbox/* is documented: read the treasury, mint money, settle a payin and move a payout."
---
...
The sandbox is the same API against one emulated partner on Base Sepolia. Same paths, same bodies, same slugs, same signatures — only the counterparty is fake, and four extra routes let you play it: the 44 operations production serves, plus these.
...
| Fiat partner | One emulator answers for every rail. It issues checksum-valid requisites — `SBOX…` BICs, DE/AE/GB IBANs, ABA routing numbers, a PIX key, an 18-digit SPEI number. |
| Money in and out | Paid by a shared HEVN treasury on demand, through the routes below. Nothing arrives on its own. |
...
Three of these four routes move a client's money, and the header is how you pick which client. `GET /v1/sandbox/treasury` accepts it and reports the same shared treasury either way.
...
Being an integrator, being allowed to create clients, and being allowed to operate escrow are flags on your account, and nothing in the API sets them — in the sandbox as in production, HEVN does. There is no self-enrolment call. Until the flags are on, `POST /v1/clients` answers `403 integrator_inactive` or `403 client_creation_not_enabled`; `GET /v1/clients` answering an empty page is the cheapest confirmation that they are.
...
`maxCredit` is the live per-call ceiling. When the treasury is empty or out of gas, credits answer `503 provider_unavailable` with `details.treasuryAddress` and `details.token` — that is a faucet problem, not your bug.

## Mint money

`POST /v1/sandbox/deposits` has two arms, and `bankId` picks between them.

- **With `bankId`** it is an unsolicited bank transfer into that rail's requisites: `amount` is in the rail's currency, the emulator converts at the fixed rate and delivers the settlement token to the client's wallet. `remitter` and `paymentReference` are the payer's name and message.
...
A bank deposit answers the same shape with the fiat side filled in — `amount` and `currency` in the rail's currency, `credited` in the settlement token, plus `fee`, `feeCurrency` and `rate`. When the credit settles a payin you quoted, `payinId` names it.
...
`202` means the treasury's transfer has not confirmed yet. Repeat the same request until it is `settled`; nothing is ever credited twice.
...
The body is optional and `remitter` is the only field in it — it becomes the payer name on the resulting income row, which is what you want if you are testing reconciliation. The payin must be open: one you never opened with `POST /v1/payins/{payinId}/instructions` answers `409 quote_not_submitted`.
...
| `refunded` | yes | The status flips, then the treasury returns the source token to the client's wallet. |
...
Three things the production path makes you wait for happen here without a call: a submitted KYB application is approved immediately, a rail you open becomes `active` with requisites as soon as the partner is asked, and every row exists by the time the call returns. That is the difference that will bite when you switch hosts — [Going live](/whitelabel/going-live) is the list.
