...
- **With `bankId`** it is an unsolicited bank transfer into that rail's account details: `amount` is in the rail's currency, the emulator converts at the fixed rate and delivers the settlement token to the client's wallet. `remitter` and `paymentReference` are the payer's name and message.
