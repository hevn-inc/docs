...
1. **Your session is still allowed to ask.** Every request re-reads the developer-key registration
   behind your token: it must still be active, and the request must come from an address inside that
   registration's immutable allowlist. HEVN checks both again immediately before it co-signs.
2. **You sign.** Your developer key is registered as a signer for the wallet, and signs the exact
