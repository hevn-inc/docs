...
A rail is a bank route you request for one client. Money wired to its requisites is converted **by the partner bank** and delivered to that client's wallet.
...
The choice that is yours per payment is on the money-in side. `POST /dapi/v1/payins` takes `settlementToken` and quotes the deposit in it; when no rail of the client's settles into the token you asked for, the refusal carries `details.options[]` with the pairs that do ([Payins](/whitelabel/payins)). Nothing on this page converts an existing balance — a settlement token only ever describes money that has not arrived yet.
