...
description: "Fill the client's KYB document, upload its papers, submit it for review, and answer an RFI — all from your backend."
---
...
```mermaid
flowchart LR
    G1["1 · Client ready<br/>GET /v1/clients/{clientId}"]
    G2["2 · Phone set and document complete<br/>PUT /v1/clients/{clientId}/kyb → ready: true"]
    G3["3 · Application approved<br/>GET /v1/clients/{clientId}/kyb"]
    G4["4 · Rail active<br/>GET /v1/banks"]
    G1 --> G2 --> G3 --> G4
...
Every route here names the client in its path, so you do not send `X-Hevn-Account` with it — the one exception is document upload, which is a client-scoped route and does. Your own access token is the only credential any of this needs. Examples use the client from [HTTP client](/whitelabel/reference/client).
...
```mermaid
flowchart LR
    W["PUT /v1/clients/{clientId}/kyb"] --> R{"ready?"}
    R -->|"false"| F["Fix the paths in missing[]"]
    F --> W
    R -->|"true"| S["POST /v1/clients/{clientId}/kyb/submit"]
```
...
Send what you know. Required values, their accepted enum members and the exact spelling of each field live in the [KYB field reference](/whitelabel/reference/kyb-fields).
...
```bash cURL
curl -X PUT "$HEVN_API/clients/$HEVN_CLIENT_ID/kyb" \
...
```python Python
readiness = hevn.put(f"/clients/{client_id}/kyb", document)
```

```javascript Node
const readiness = await hevn.put(`/clients/${clientId}/kyb`, document);
```

```go Go
readiness, err := api.Put("/clients/"+clientID+"/kyb", document)
```
...
Upload a file once, then file its id into a slot. `POST /v1/documents` takes JSON, with the file base64-encoded in `content` and the slot in `slot`. Uploads are scoped to the client, so send `X-Hevn-Account`.
...
`GET /v1/clients/{clientId}/kyb` returns readiness, the application if one is filed, and the whole document projected back, so you can diff your own state against HEVN's:
...
`ready: true` means submission will succeed. Nothing else does — the list is deliberately stricter than the minimum a reviewer would accept, so that a document that reads ready never fails at the gate.
...
```bash cURL
curl -X POST "$HEVN_API/clients/$HEVN_CLIENT_ID/kyb/submit" \
  -H "Authorization: Bearer $HEVN_ACCESS_TOKEN"
```

```python Python
application = hevn.post(f"/clients/{client_id}/kyb/submit")
```

```javascript Node
const application = await hevn.post(`/clients/${clientId}/kyb/submit`);
```

```go Go
application, err := api.Post("/clients/"+clientID+"/kyb/submit", nil)
```

</CodeGroup>

No body. `201 Created`, `Location: /v1/clients/cl_7YQ2Kf3mN8/kyb`:
...
```json
{
  ...
  "status": "pending",
  "attemptNo": 1,
  "pollUrl": "/v1/clients/cl_7YQ2Kf3mN8/kyb"
}
```

Submitting again with nothing changed replays: `200` with `Idempotency-Replayed: true` and the same application id. Submitting an incomplete document answers `422 kyb_incomplete` with the same `details.missing` list the document read returns.
...
Poll `GET /v1/clients/{clientId}/kyb`. There is no webhook; `kybStatus` on the [client row](/whitelabel/clients) carries the same value if you are already reading that.

| `status` | What it means | What you do |
| --- | --- | --- |
| `notStarted` | No application filed yet. | Complete the document and submit. |
| `pending` | Filed and under review. | Poll. Minutes to days, depending on the jurisdiction and the papers. |
| `approved` | The client is verified. | Open its rails. |
| `rfi` | The reviewer asked for something. | Read `application.comment`, fix the document, submit again. |
...
An RFI is the same loop, once more. Read `application.comment`, write the correction into the document, and submit again.

The correction is a normal write. A replacement paper is a new upload filed into the slot it belongs in — a new upload id in an occupied slot replaces what was there, so there is nothing to delete first. A wrong date of birth is a `PUT` with that person's `key` and the right value. Then submit: `attemptNo` increments, `comment` is replaced by the next reviewer's, and `kybStatus` returns to `pending`.

Two refusals to expect at submit time:
...
| `422 document_fields_missing` | The papers are attached, but HEVN could not read the values a reviewer needs off one of them. | `details.documents` names each gap. Fill them with `PUT /v1/documents/{documentId}/content`, then submit again. |
...
| --- | --- | --- |
| Account phone | `POST /v1/clients` or `PATCH /v1/clients/{clientId}` | The rail requirements check. A company with no account phone reports the `phone_required` blocker on every rail. |
| `company.phone` | `PUT /v1/clients/{clientId}/kyb` | The KYB gate. Either phone satisfies it. |
...
<Card title="Next: Bank accounts" icon="arrow-right" href="/whitelabel/bank-accounts">
