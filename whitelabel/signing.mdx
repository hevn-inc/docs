...
`publicKey` is optional; it only saves HEVN a scan over your registered keys. A key that is not one
...
The credential is re-checked at both ends. Every request on this API re-reads the developer-key
registration behind your session and the address the request came from, and the confirm re-checks
them once more immediately before HEVN co-signs — so a registration paused between the prepare and
