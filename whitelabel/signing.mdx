...
<Steps>
  <Step title="Prepare" icon="file-text">
    You call the opening route — `POST /v1/payouts`, `POST /v1/transfers`,
    `POST /v1/escrow/{escrowId}/actions`. HEVN prices the payment, books it, builds the on-chain
...
sequenceDiagram
    ...
    S->>H: POST /v1/payouts — contactId, amount, publicKey
    ...
    loop safe to repeat — never re-open unless it expired
        S->>H: POST /v1/payouts/{payoutId}/confirm — signature
...
| --- | --- | --- | --- |
| Fiat out over a bank rail | `POST /v1/payouts` | `POST /v1/payouts/{payoutId}/confirm` | 120 s |
| USDC out on Base | `POST /v1/transfers` | `POST /v1/transfers/{transferId}/confirm` | 120 s |
| An escrow deal, and every escrow action | `POST /v1/escrow`, `POST /v1/escrow/{escrowId}/actions` | `POST /v1/escrow/{escrowId}/actions/{idempotencyKey}/confirm` | 60 s |

Payouts and transfers act on a client — send `X-Hevn-Account`. Escrow is always operated by you, so it
...
`publicKey` is optional; it only saves HEVN a scan over your registered keys.
...
- checks that your account controls the client's wallet and that the wallet's live on-chain owner is
  your signer;
- builds a **single stablecoin transfer** from the client's wallet, with the amount and recipient
...
- **Five attempts per payment.** After that a payment answers `funding_attempts_exhausted` and needs
  a new one.
...
| `409` `funding_attempt_expired` | The approval expired | Re-open with the same `Idempotency-Key` |
| `409` `funding_state_unknown` | An earlier attempt may still land | Stop. Contact support with the payment id |
| `400` `signature_invalid` | Wrong bytes or wrong encoding | Fix the signing and confirm again |

<Warning>
  After a `202`, a `502` or a timeout, do not re-open the payment: an earlier attempt may still land.
  Re-open only on an explicit `failed`, `funding_attempt_expired`, or a `409` that says so.
</Warning>
