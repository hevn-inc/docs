...
description: "Check what a rail still needs, open a virtual account for a client, read the account details its payers wire to, and know when it is payable."
---

import ActingAsSnippet from "/snippets/acting-as.mdx";

A **rail** is a bank route you can request for one client. Opening it gives that client a **virtual account** — a `bnk_…` record in the client's own legal name — and the virtual account publishes **account details**: the IBAN, the account and routing number or the PIX key its payers wire to. Money that arrives there is converted **by the partner bank** and delivered to the client's wallet.

Three levels, three names, and they stay put for the rest of these guides: the rail is the route you ask for, the virtual account is what you get, the account details are what you hand the payer. On the wire the details arrive in a field called `requisites`.

Opening one is two calls — ask what it needs, then ask for it. The rail id in both comes from `GET /dapi/v1/banks`, which you read first:
...
```mermaid
stateDiagram-v2
    [*] --> preparing: POST /dapi/v1/banks
    preparing --> active: account details published
    ...
    note right of preparing
        ...
        account details published yet. Not payable.
    end note
    note right of active
        Account details present. Payable.
    end note
```

A rail is payable when `status` is `active`. That is the whole rule — `active` is only reported once the account details exist, so there is no second condition to check and no client-side branch to write. A rail you have not opened has no `status` at all.
...
<Note>
  In production a rail is opened by review, not by your call. `POST /dapi/v1/banks` answers `201` with `status: "preparing"` and the rail becomes `active` when the partner publishes the account details — hours to days, depending on the rail and the client's jurisdiction. In the sandbox that review is emulated and the rail activates at once.
</Note>

## Find the rail

Rail ids are opaque strings. You copy them out of `GET /dapi/v1/banks`, which returns the rails this client may be paid over and which of them are already open:
...
The list is the client's own catalogue: rails closed to its residency are not in it. Pick by `currency` and `method`, never by parsing the id — what each method means and which currency it carries is in the [rail reference](/whitelabel/reference/rails). `GET /dapi/v1/banks/{rail}` returns one record in the same shape.

<Note>
  The rail ids printed on this page stand in for real ones. Ids differ per environment and per partner, and the only correct source is the `GET /dapi/v1/banks` you just ran for this client — which is why every call above takes `$HEVN_RAIL` rather than a literal.
</Note>
...
| `code` | Cleared by |
| --- | --- |
| `phone_required` | A phone number on the account — `POST /dapi/v1/clients`, or `PATCH /dapi/v1/client` with the id in `X-Hevn-Account`. The phone inside the KYB document does not clear it. |
...
`fields` carries only the identifiers this method uses — an IBAN for SEPA, an account and routing number for ACH, and so on; the full matrix is in the [rail reference](/whitelabel/reference/rails). The shape is `RailRequisitesView` on a rail and `ContactPaymentAccountView` on a [contact](/whitelabel/payouts#save-a-contact); both carry the same `fields`, `holder` and `bankName`.
...
On a `named` rail the partner bank opens the account in the client's own legal name, so the payer's bank shows the client and nothing about HEVN. Hand your customer the `fields`, the `holder` name and `bankName` exactly as they come back, and re-read them before each use rather than caching them — a partner can republish an account.
...
`supportedAccounts` on the rail record is what this rail can deliver into the client's wallet, and `destinationAccount` is what it delivers today. Neither is something you set after the fact: **the rail decides, and the rail is fixed when you open it.** A rail that can deliver two tokens ships as two rail ids, one per token, so choosing the token is choosing which rail to open with `POST /dapi/v1/banks` — see [Rails](/whitelabel/reference/rails).

Read both back, never cache them:

```json
{ "rail": "sepa_named-bank_a", "supportedAccounts": ["EURC"], "destinationAccount": "EURC" }
```

The choice that is yours per payment is on the money-in side. `POST /dapi/v1/payins` takes `destinationAccount` and quotes the deposit in it; when no rail of the client's settles into the token you asked for, the refusal carries `details.options[]` with the pairs that do ([Payins](/whitelabel/payins)). Nothing on this page converts an existing balance — `destinationAccount` only ever names where money that has not arrived yet will land.
