...
<CardGroup cols={2}>
  ...
  <Card title="Money movement" icon="arrow-right-left" href="/cli/commands/money-movement">
    Check balances, create deposit quotes, pay invoices, and transfer to contacts.
  </Card>
  <Card title="Contracts" icon="file-pen-line" href="/cli/commands/contracts">
...
One `hvn_...` API key is used for every authenticated endpoint, including transfers. Each API key is a full EVM keypair whose spending ability is bounded by an onchain spend permission granted by the account owner — see [how API keys work](/general/authentication#api-keys-cli-rest-api-agents). The key is always sent in the `X-Api-Key` header.
...
<Note>
  App API keys are the CLI and agent credential. They are not the credential for the [whitelabel REST API](/api-reference/overview), which authenticates with a developer key and JWT sessions and rejects `X-Api-Key`.
</Note>
