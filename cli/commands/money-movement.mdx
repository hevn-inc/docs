...
Four things live on this page and they run in order: what the account holds, how money gets in,
what has happened so far, and how money gets out. Every transfer command accepts
`--idempotency-key`, and a transfer is the one place where repeating a command without one costs
real money.
...
Incoming bank transfers settle directly against the virtual account details on your open rail — see [Deposits & fiat payouts](/general/deposits-and-payouts). Check pending bank deposits:
...
<Card title="Next: Invoices" icon="file-text" href="/cli/commands/invoices">
  Create, upload, batch and pay invoices.
</Card>
