...
This is the operating guide an agent follows before it acts on a HEVN account. Four rules cover most of it: read the CLI contract before you plan, ask for structured output, read a resource before you write it, and never invent a value you were not given.

<Note>
  Using an MCP coding agent like Claude Code, Codex, or Cursor? Run [`hevn mcp install`](/cli/connect-agents) to expose every command as an MCP tool — the guidance below is delivered to the agent automatically.
</Note>
...
<Card title="Next: Connect AI agents" icon="plug" href="/cli/connect-agents">
  Register the CLI as an MCP server so an agent gets all of this automatically.
</Card>
