...
Everything the CLI needs lives in one local config file and a handful of environment variables.
Nothing here is required to get started — `hevn login` writes what it needs — but each setting is
the answer to one question a script eventually asks.
...
The key is always sent in the `X-Api-Key` header — see [how API keys work](/general/authentication#api-keys-cli-rest-api-agents). The CLI does not use `HEVN_APP_ID`; transfer endpoints derive the owning app from the API key.
...
<Card title="Next: Account and profile" icon="arrow-right" href="/cli/commands/account">
  Inspect the active account, update profile fields, and check KYC status.
</Card>
